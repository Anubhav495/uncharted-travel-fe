...
const FeatureCard: React.FC<{
    ...
    return (
        <a 
            href={href} 
            className="group block relative h-64 sm:h-80 md:h-96 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-yellow-400/50"
        >
            <img 
                src={imageUrl} 
                alt={title}
            ...
            <div className="relative h-full flex flex-col justify-end p-4 sm:p-6 text-white">
                ...
                <p className="text-sm sm:text-base opacity-90 mb-3">{description}</p>
                <span className="text-sky-300 font-semibold group-hover:underline text-sm sm:text-base">
                    Learn More →
                </span>
            </div>
        </a>
